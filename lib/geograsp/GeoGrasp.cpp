...
GraspPose GeoGrasp::getBestGraspPose() const {
    ...
    if (this->graspPoints.empty())
        ...
        grasp.firstPoint[0] = this->graspPoints[0].firstPoint.x; // graspPoints: vector of structs, we get x of the first point in first struct
        ...
        Eigen::Vector3f objAxisVector(this->objectAxisCoeff->values[3],       // main axis (V)
                                      this->objectAxisCoeff->values[4],
                                      this->objectAxisCoeff->values[5]);
        Eigen::Vector3f axeX, axeY, axeZ;

        axeZ = grasp.firstPoint - grasp.secondPoint;       // z-axis points from q1 to q2
        axeX = axeZ.cross(objAxisVector);             // x-axis is normal on both z,v
        ...
        grasp.midPointPose.linear() = midPointRotation;  // linear (matrix 3x3)
    }
...
void GeoGrasp::compute() {
    ...
    const float voxelRadius = this->gripTipSize / 2000.0;     // in m
    float graspRadius = 2.0 * this->gripTipSize / 1000.0;     // this is the Sphere size in m
...
void GeoGrasp::getBestGraspingPoints(
    ...
    for (size_t i = 0; i < firstNormalCloud->points.size(); ++i) {
        ...
        float pointCentroidDistance = (thisVector - centroidVector).norm();
