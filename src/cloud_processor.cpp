...
//****************** Now we have table in a cloud called (cloudPlane) and the rest in a cloud called (cloud)
    ...
    ecExtractor.setMinClusterSize(450);
    ...
    cout << "Found " << clusterIndices.size() << " clusters!" << endl;
    return;
...
int main(int argc, char **argv) {
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("../data/YCB_PCD_Files/master_chef_can.pcd", *pointCloud) == -1)
    {
