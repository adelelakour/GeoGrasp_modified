...
struct GraspContacts {              //q1,q2 with.r.t Camera frame
  pcl::PointXYZ firstPoint;
  pcl::PointXYZ secondPoint;
};

struct GraspPose {
  Eigen::Vector3f firstPoint;    // where thumb tip should be located
  Eigen::Vector3f secondPoint;   // where middle tip should be located
  Eigen::Affine3f midPointPose;  // position/orientation of hand_frame
};
